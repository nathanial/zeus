...
impl Evaluator {
    ...
    fn eval_progn(&mut self, list: &[Expr]) -> Result<Expr, String> {
        if list.len() == 1 {
    ...
    fn eval_case(&mut self, list: &[Expr]) -> Result<Expr, String> {
        if list.len() < 2 {
    ...
    fn expr_equal(&self, a: &Expr, b: &Expr) -> bool {
        match (a, b) {
    ...
    fn format_expr_for_print(&self, expr: &Expr) -> String {
        match expr {
            Expr::Number(n) => {
